...
def trade(context, data):
    if context.model:
        ...
        context.short_candidates = df_prediction[df['gap'] < 0].head(5).index.values
        context.long_candidates = df_prediction[df['gap'] > 0].tail(5).index.values
...
def close_orders(context, data):
    for sid in context.portfolio.positions:
        ...
        logging("{0}\t{1}\t{2: .5f}\t{3: .5f}\t{4}\t{5: .5f}\t{6: .5f}\t{7}\t{8: .5f}\t{9: .5f}\t{10: .5f}".format(side, sid.symbol, gap, pl, sector, close_price, pipeline_close_price, opentime,amount, openprice, currentprice))
