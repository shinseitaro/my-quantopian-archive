...
def initialize(context):
    ...
    # ショートポジション 金曜日のクローズ
    schedule_function(my_rebalance_position_open1, date_rules.week_end(2), time_rules.market_close())
    # ポジションクローズ　水曜日のオープン
...
def my_record(context, data):
    ...
    # log.info('Positions on %s is %s' %( get_datetime('US/Eastern'), context.portfolio.positions.positions_value))
    record(vxx=data.current(context.vxx, 'price'))
    record(vxx=data.current(context.vxz, 'price'))
