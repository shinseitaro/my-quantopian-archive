...
def initialize(context):
    ...
    # ポジションクローズ　水曜日のオープン
    schedule_function(my_rebalance_position_close, date_rules.week_start(3), time_rules.market_open())
...
def my_rebalance_position_open_vxx(context,data):
    ...
    if data.can_trade(context.vxx):
        context.order_id = order_percent(context.vxx, -0.2) #order_value(context.vxx, -1000)
        logging('VXX Short Position Opened: %s @ %s: Total %s' %( 
                 get_order(context.order_id).amount, data.current(context.vxx, 'price'),
                 get_order(context.order_id).amount * data.current(context.vxx, 'price')))
        context.traded_dates += 1    
        
    logging('Traded Date Count %s / %s (%s) ' % 
            (context.traded_dates, context.tradabale_dates, context.traded_dates/context.tradabale_dates * 100.0))
