...
def initialize(context):
    ...
    schedule_function(my_rebalance, date_rules.every_day(), time_rules.market_open(minutes=4))    
...
def create_model_data(context, data, bar_count, train=True):
    ...
    if train:
        ## .between_time("09:32", "10:10")のDFを渡す事で，必ず09:32が先頭にくる．
        df_price_btw_0932_1010 = at_two_times(pan_historical_data['price'].fillna(method='ffill').between_time("09:32", "10:10") , "09:32", "10:10")
    ...
    ## panは，itemが辞書のキー，majorindexが sid, minorindex が int のデータなので，
    ## to_frameして itemをコラム，major/minor index を multiindex に持つ DataFrameを作り
    ## .reset_index(drop=True, level=0)で，minor index だけを捨てる．
    ## 注意；pan.to_frame() は
    # df = pan.to_frame(filter_observations=False)#.reset_index(drop=True, level=0)
    #df = pan.to_frame().reset_index(drop=True, level=0)
    # if train:
    #     df = pan.to_frame().reset_index(drop=True, level=0)
    # else:
    #     df = pan.to_frame(filter_observations=False).reset_index(drop=True, level=0)
    #print df[['gap', 'return', 'turnover_ratio']]
    print len(df)
    print len(df.dropna())
...
def trade(context, data):
    if context.model:
        ...
        X = test_data.values
        sids = test_data.index
        prediction = context.model.predict(X)
        df_prediction = pd.DataFrame(prediction, index=sids, columns=["return"]).sort_values(by="return")
        context.short_candidates = df_prediction.head(5).index.values
        context.long_candidates = df_prediction.tail(5).index.values
