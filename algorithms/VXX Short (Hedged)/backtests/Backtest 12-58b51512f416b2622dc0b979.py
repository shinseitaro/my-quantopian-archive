...
def my_rebalance_position_open(context,data):
    ...
    if(data.can_trade(context.vxx)) & (data.can_trade(context.vxz)):
        ## vxz は　volume が小さくかつ， quantopian のバックテストは実際に行われたトレードの1/40のサイズをマックスにしてあるため，
        ## しばしば，partially filled が起きるので，それを避ける為に vxz で実際取引されたamount 分， vxz をトレードする
        ## しかし，これはあくまでバックテスト用で，デモトレードや本番で行うのはかなり難しい
       
        context.order_id2 = order_percent(context.vxz, 0.5)
        logging(get_order(context.order_id2).amount)
        if get_order(context.order_id2).amount > 0:
            context.order_id = order_value(context.vxx, get_order(context.order_id2).amount * data.current(context.vxz, 'price') * -1)
