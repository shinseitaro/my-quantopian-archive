...
def my_rebalance_position_open(context,data):
    ...
    if(data.can_trade(context.vxx)) & (execdate.day <= 25):
        ...
        log.info('VXX Short Position Opened: %s @ %s' % (get_order(context.order_id).amount,
                                                         data.current(context.vxx, 'last_traded')))
        log.info('VXZ Long Position Opened: %s @ %s' % (get_order(context.order_id2).amount,
                                                         data.current(context.vxz, 'last_traded')))
