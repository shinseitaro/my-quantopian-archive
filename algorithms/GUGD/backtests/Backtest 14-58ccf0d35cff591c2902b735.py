...
def handle_exit(context, data, at_market_closing = False):
    ...
    if sids:
        for sid in sids: 
            ...
            if go:
                order_percent(sid, 0)
                msg = msg + "\t symbol \t {symbol} \tClosed \t {current} \tTraded at \t {last_sale_price} \t prevclose \t {prevclose} \t today's open \t{todayopen}\tGap \t {gap}\t PL \t {pl}\t SP \t{sp}"
                msg = msg.format(symbol=sid.symbol, current=current, 
                                 last_sale_price=last_sale_price, 
                                 gap=gap, pl=current/last_sale_price-1,
                                todayopen=todayopen, prevclose=prevclose, sp=context.spx_open_ratio)
