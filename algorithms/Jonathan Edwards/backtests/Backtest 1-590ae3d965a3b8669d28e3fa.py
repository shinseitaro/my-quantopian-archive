...
def initialize(context):
    context.symbols = symbols('JNUG', 'NUGT', 'JDST', 'TZA', 'DUST', 'UGAZ', 'DGAZ', 'SPXU', 'LABD', 
                              'SPXS', 'FAS', 'TNA', 'DWT', 'SQQQ', 'TQQQ', 'LABU', 'ERX', 'FAZ', 'UPRO', 'USLV', 'ERY', 'TMV', 'SDOW', 'EDZ')
...
def my_rebalance(context,data):
    ...
    for s in context.longs:
        ...
        if data.can_trade(s) and posi.amount == 0:
            order_id = order_target_percent(s, 1.0 / len(context.sid_list))

    for s in context.shorts:
        posi = context.portfolio.positions[s]
        if data.can_trade(s) and posi.amount == 0:
            order_id = order_target_percent(s, 1.0 / len(context.sid_list) * -1)

            
    for s in context.portfolio.positions:
        posi = context.portfolio.positions[s]
        if data.can_trade(s) and posi.amount > 0: # long posion
            if data.current(s, 'price') / posi.cost_basis - 1 > 0.05:
                print("LONG POSITION CLOSE:", s.symbol)
                order_target_percent(s, 0)
        if data.can_trade(s) and posi.amount < 0: # short posion
            if posi.cost_basis / data.current(s, 'price')  - 1 > 0.05:
