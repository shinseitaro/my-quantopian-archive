        ...
        for sid in sids: 
            ...
            if context.order_ids[sid]['position'] != 0:
                ...
                if at_market_closing:
                    msg = "Order Closed symbol {symbol} At Market Closing: Price {current}: Bought at {last_sale_price} : Gap {gap}: PL {pl}"  
                    go = True
                else:
                    if current >= target_price:
                        go = True
                        msg = "Order Closed symbol {symbol} : Price {current}:  Bought at {last_sale_price} : Gap {gap}: PL {pl}"    
            if go:
                order_percent(sid, 0)
                msg = msg.format(symbol=sid.symbol, current=current, 
                                 last_sale_price=last_sale_price, gap=gap, pl=current/last_sale_price-1)
                #logging(msg % (sid.symbol, current, gap, current/target_price-1))
                logging(msg)
