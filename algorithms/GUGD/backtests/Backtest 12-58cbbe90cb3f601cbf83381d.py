...
def handle_entry(context, data):
    ...
    for sid in context.stocke_list:
        ...
        if (-0.02 <today_open[sid][-1] / prev_close[sid][-2] - 1 < -0.01) & (today_open[sid][-1] <= 10):
            ...
            if context.order_ids[sid]['position'] != 0:
                ...
                if at_market_closing:
                    ...
                    if current >= target_price: #current / last_sale_price - 1 > 0.01:
                        go = True
                        msg = "Order Closed" 
                    elif current / last_sale_price - 1 < -0.01: ## 損切り
                        go = True 
                        msg = "Order Closed (Loss Cut)" 
                        
            if go:
                order_percent(sid, 0)
                msg = msg + "\t symbol \t {symbol} \tPrice \t {current} \tBought at \t {last_sale_price} \t prevclose \t {prevclose} \t today's open \t{todayopen}\tGap \t {gap}\t PL \t {pl}"
