...
def my_rebalance(context, data):
    ...
    # ストラテジーには使っていないが，nearの満期まであと何日（営業日ベース）残っているか出力
    ...
    if target_weights:
        # order_target_percent(f3_contract, target_weights[f3_contract])
        # order_target_percent(f5_contract, target_weights[f5_contract])
        order_optimal_portfolio(objective=opt.TargetWeights(target_weights),
                                constraints=[opt.MaxGrossExposure(5.0)])     
