...
def ordering_logic(context, data,):
    ...
    if context.vxx_weight == context.vxz_weight == 0:
    ...
    elif (vxx_weight == context.vxx_weight) & (vxz_weight == context.vxz_weight):
        ...
        num = int(amount / price) if amount != 0 else 0
        orderid = order_value(context.vxx, 0)
        log.info('Closed %s: %s shares for %s @ %s' % (context.vxx.symbol, num, amount, price))
        ...
        log.info('Closed %s: %s shares for %s @ %s' % (context.vxz.symbol, num, amount, price))
...
def rebalance(vxx_weight, vxz_weight, context, data):
    ...
    if data.can_trade(context.vxx) & data.can_trade(context.vxz):
        ...
        if (abs(vxx_order_value) > vxx_current_value) & (vxz_order_value > vxz_current_value): 
            ...
            log.info('VXX Short: %s, commission: %s ' % (get_order(order_id).amount,
                                                        get_order(order_id).commission)) 
            
            order_id = order_value(context.vxz, vxz_order_value)
            log.info('VXZ Long %s, commission: %s ' % (get_order(order_id).amount,
                                                        get_order(order_id).commission)) 
