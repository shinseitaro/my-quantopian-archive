...
def initialize(context):
    ...
    context.trade1 = 0
    context.trade2 = 0
    ## 月曜Openでポジションショート，火曜オープンでポジションクローズ
    ...
    #金曜のクローズでポジションショート月曜のオープンでクローズ
    schedule_function(rebalance_friday, date_rules.week_start(), time_rules.market_close())
    schedule_function(rebalance_monday_close, date_rules.week_start(), time_rules.market_close())
    
    #金曜のクローズでポジションショート火曜のオープンでクローズ
    # schedule_function(rebalance_friday, date_rules.week_start(), time_rules.market_close())
    # schedule_function(rebalance_tuesday, date_rules.week_start(days_offset=1), time_rules.market_open())
...
def rebalance_monday_close(context,data):
    if (data.can_trade(context.vxx)) & (context.portfolio.positions[context.vxx].amount != 0):
        order_percent(context.vxx, 0)
        log.info('Closed VXX') 
    else:
...
def rebalance_friday(context,data):
    order_percent(context.vxx, -1.0)
    log.info('VXX Short Position Open')
