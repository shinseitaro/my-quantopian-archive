...
def my_rebalance_position_open_vxz(context,data):
    ...
    context.tradabale_dates = context.tradabale_dates + 1
    ...
    if data.can_trade(context.vxz):
        context.traded_dates = context.traded_dates  + 1   
        #context.order_id2 = order_percent(context.vxz, 0.3)
        context.order_id2 = order_value(context.vxz, 1000)
    logging('Traded Date Count %s / %s (%s) ' % 
            (context.traded_dates, context.tradabale_dates, context.traded_dates/context.tradabale_dates * 100))
    
def my_rebalance_position_open_vxx(context,data):
    
    if context.portfolio.positions[symbol('VXZ')].amount > 0: 
...
def my_rebalance_position_close(context, data):
    ...
    context.order_id2 = order_target(context.vxz, 0.0)
