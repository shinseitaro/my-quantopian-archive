...
def initialize(context):
    context.symbols = symbols('TECS', 'TWM', 'TZA', 'YANG', 'DUST', 'DXD', 'FAZ', 'FXP', 'GASX',  'MIDZ', 'MZZ', 'QID', 'JDST', 'TVIX', 'BIS',  'UVXY', 'DGAZ', 'SOXS', 
                  'RUSS', 'FAZ', 'MIDZ', 'SMDD', 'BZQ', 'DPK', 'SSG', 'ERY', 'DWT', 'SPXS', 'SCO', 'SRTY', 'TZA', 'SIJ', 'EDZ', 'DUG', 'SPXU', 'SMN', 'TECS', 'SKF', 'EEV', 'TWM', 'MZZ',
                   'SQQQ', 'SDP', 'SDOW', 'EPV', 'TTT', 'TMV', 'QID', 'EFU', 'EWV', 'SDS','TBT', 'DZZ', 'HDLV', 'EUO', 'TPS', 'TBZ',)
...
def my_rebalance_for_exit(context,data):                
    for s in context.portfolio.positions:
        ...
        if data.can_trade(s) and posi.amount > 0: # long posion
            if (data.current(s, 'price') / posi.cost_basis - 1 > 0.025): #or (data.current(s, 'price') / posi.cost_basis - 1 < -0.025):
                print("LONG POSITION CLOSE:", s.symbol)
                order_target_percent(s, 0)
                
        if data.can_trade(s) and posi.amount < 0: # short posion
            if (posi.cost_basis / data.current(s, 'price')  - 1 > 0.025): # or (posi.cost_basis / data.current(s, 'price')  - 1 < -0.025):
