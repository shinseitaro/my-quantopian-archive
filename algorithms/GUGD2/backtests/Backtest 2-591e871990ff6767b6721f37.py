...
def initialize(context):
    ...
    schedule_function(my_rebalance_close, date_rules.every_day(), time_rules.market_open(minutes=context.bar_count)) 
...
def find_gapdown(context, data): #results, date, turnover_threshold=0.05, gapdown_threshold = 0.0
    ...
    pan = data.history(sids,['price', 'volume'], 1, frequency='1m') #get_pricing(sids, start_date=date, end_date=date, frequency='minute') 
...
def my_rebalance(context,data):
    ...
    for sid in context.gapdowns:
        #logging("gap down: %s return: %s" % (sid.symbol, context.output.close_to_close.ix[sid]))
        
        order_percent(sid, 1.0/cnt)
    for sid in context.gapups:
        #logging("gap up: %s return: %s" % (sid.symbol, context.output.close_to_close.ix[sid]))
        order_percent(sid, -1.0/cnt)
...
def my_rebalance_close(context, data):
    for sid in context.gapdowns:
        order_percent(sid, 0)
        logging("PL\t%s\t%s" % (sid.symbol, return_at_position_closed(context, data, sid)))
