...
def find_gap_sids(context, data): #results, date, turnover_threshold=0.05, gapdown_threshold = 0.0
    
    sids = context.output.index
    pan = data.history(sids,['price', 'volume'], 1, frequency='1m') 
    pan['turnover'] = pan.price * pan.volume

    df_today_0931_close = pan.price.ix[0] 
    df_today_0930_turnover = pan.turnover.ix[0]
    df_prevday_close = context.output.yesterday_close
    df_prevday_turnover = context.output.yesterday_turnover
    
    df = pd.DataFrame({'gap': (df_today_0931_close / df_prevday_close - 1),
                       'turnover': (df_today_0930_turnover / df_prevday_turnover)})
    
    df_gaps = df[df.turnover > context.turnover_threshold]
    if context.gapdown_threshold_upper:
        df_gaps = df_gaps[df_gaps.gap < context.gapdown_threshold_upper]
    if context.gapdown_threshold_lower:
        df_gaps = df_gaps[df_gaps.gap > context.gapdown_threshold_lower]    
    
    df_gaps = df_gaps.sort_values(by=['gap','turnover'], ascending=[True,False])
    
    context.gapdowns = df_gaps.index
    ...
    if context.gapdowns.any():
        ...
        if context.gapdowns.any():
            context.gapdown_ratio = df_gaps.ix[context.gapdowns]['gap']
...
def find_gapdown(context, data): #results, date, turnover_threshold=0.05, gapdown_threshold = 0.0
    ...
    df_gapdowns = df[(df.turnover > context.turnover_threshold) & 
                     # (df.gap < context.gapdown_threshold_upper) & 
                     (df.gap > context.gapdown_threshold_lower)]    
    ...
    if context.gapdowns.any():
        ...
        dscr = df_yesterday[:-1].fillna(method='ffill').fillna(method='backfill').pct_change().dropna().describe() 
        context.gapdowns = dscr.loc[:, dscr.loc['mean'] > 0].T.index
        ...
        if context.gapdowns.any():
            logging(",".join(sorted([s.symbol for s in context.gapdowns])))
