...
def initialize(context):
    ...
    fetch_csv('https://dl.dropboxusercontent.com/u/264353/updates-2017-02-27.csv',  
              pre_func=preprocess,  
              date_column='DATE',  
              symbol = 'FOMC',
              )  
...
def my_rebalance_position_open(context,data):
    ...
    if(data.can_trade(context.vxx)) & (execdate.day <= 25):
        ...
        context.trade_price = data.current(context.vxx, 'price')
        logging('VXX Short Position Opened: %s @ %s ' %
                (get_order(context.order_id).amount, context.trade_price))
        context.is_trade_open = True
...
def my_rebalance_position_open1(context,data):
    ...
    if not context.go:
        logging('No Trade because ivts %s < 2 ' % context.ivts)
...
def my_rebalance_position_open2(context,data):
    ...
    if not context.go:
        logging('No Trade because ivts %s < 0.91 ' % context.ivts)
    if(data.can_trade(context.vxx)) & (execdate.day <= 25) & context.go:
        context.order_id = order_percent(context.vxx, -1.0)
...
def my_rebalance_position_close(context, data):
    tradepl = 0.0
    if context.trade_price:
        tradepl = context.trade_price/data.current(context.vxx, 'price')-1
    context.order_id = order_target(context.vxx, 0.0)
    # context.order_id = order_target(context.vxz, 0.0)
    logging('VXX Close at %s. Profit is\t%s ' % (data.current(context.vxx, 'price'), tradepl))
    
    context.is_trade_open = False
    context.trade_price = data.current(context.vxx, 'price')
    
    # logging('VXZ Close at %s: %s' % (execdate, data.current(context.vxz, 'price')))    
...
def my_record(context, data):
    ...
    record(FFRelease=data.current('FOMC', 'FOMC Interest Rate Release'))
