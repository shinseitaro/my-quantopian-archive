...
def initialize(context):
    ...
    context.lower_thred = 0.9982
    context.close_thred = 1.01
    
    context.long_boco = False

    context.order_id1 = None
...
def my_rebalance(context, data):
    ...
    multiplier = 1#contract_f1.multiplier
    ...
    ## 残存期間が残っている場合        
        ...
        if context.f2_mean_diff < context.lower_thred and not context.long_boco:
            log.info('Order: {}'.format(context.f2_mean_diff ))
            order_target(contract_f1, -1 * multiplier) 
            order_target(contract_f2, 2 * multiplier) 
            order_target(contract_f3, -1 *multiplier) 
