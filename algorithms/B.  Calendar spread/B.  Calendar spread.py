...
具体的には
(3限月/2限月) > 0 且つ (5限月/4限月) < 0 の場合，4限月をショート，2限月をロング
(3限月/2限月) < 0 且つ (5限月/4限月) > 0 の場合，2限月をショート，4限月をロング
それ以外は，ポジションを持たない
...
def initialize(context):
    ...
    context.near = context.f_2
    context.far = context.f_4
...
def get_entry_flag(context, data):
    ...
    context.near_price_contango = context.price[context.f_3] / context.price[context.f_2]
    context.far_price_contango = context.price[context.f_5] / context.price[context.f_4]
...
def my_rebalance(context, data):
    ...
    # entry_flag が True, つまり sign_nearとsign_farのフラグが逆の場合．
        ...
        if sign_near > 0:
            order_target(near, -1.0) 
            order_target(far, 1.0) 
