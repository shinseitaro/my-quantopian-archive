...
def initialize(context):
    ...
    EveryThisManyMinutes = TradingDayMinutes#10
...
def before_trading_start(context, data):
    ...
    log.debug("context.stocks_worst {}".format(sorted([s.symbol for s in context.stocks_worst])))
    # weight を計算
    context.stocks_worst_weight = my_compute_weights(context)

    # https://docs.python.jp/3/library/itertools.html#itertools.cycle
    # イテレータリストを返す．アクセスが早くなるらしいけど，ここでどのくらいアクセスが早くなるかわからん．
    ...
    # もってるポジションにたいして、現在のプライスがLowestPriceより低いか確認
    for stock in context.portfolio.positions:
        # CurrPrice = float(data.current([stock], 'price'))
        # if CurrPrice < context.LowestPrice:
        #     # 低ければ、その価格でLowestPriceを上書き
    ...
    for stock in context.age:
        if stock not in context.portfolio.positions:
            context.age[stock] = 0
            #log.info("stock in context.age but not in portforlio {}".format(stock.symbol))
...
def my_rebalance(context, data):
    ...
    for stock in context.portfolio.positions:
        if not get_open_orders(stock):
            ...
            # make_div_by_05はだいたい+１％くらいの価格を返す
            SellPrice = float(
                make_div_by_05(
                    CostBasis *
                    SellFactor,
            ...
            if np.isnan(SellPrice):
                ...
                # 銘柄がageに入っていない場合入れる
                    ...
                    context.age[stock] = 1
                # ホールドが2日以上でMyFireSaleAge以下、になったら損切りオーダーを入れる。    
                else:
                    SellPrice = float(
                    ...
                    log.info("LossCut: {stock}, {SellPrice}, {StockShares}, {age}".format(stock=stock, SellPrice=SellPrice, StockShares=StockShares, age=context.age[stock]))
            else:
                if (stock in context.age and context.age[stock] < 2):
                    pass
                elif stock not in context.age:
                    ...
                    log.info("TakeProfit: {stock}, {SellPrice}, {StockShares}, {age}".format(stock=stock, SellPrice=SellPrice, StockShares=StockShares, age=context.age[stock]))
    ...
    for ThisBuyOrder in range(context.MaxBuyOrdersAtOnce):
        ...
        if np.isnan(CurrPrice):
            ...
            #log.info("Order: {stock}, {BuyPrice}, {StockShares}".format(stock=stock, BuyPrice=BuyPrice, StockShares=StockShares))
