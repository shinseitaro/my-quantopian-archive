...
def make_pipeline(context):
    ...
    return_change_mean = ChangeAverage(inputs = [USEquityPricing.close], window_length = 3)
    volume_change_mean = ChangeAverage(inputs = [USEquityPricing.volume], window_length = 3)
...
def my_rebalance(context, data):
    ...
    if not targets.empty:
        ...
        for sid in targets.index:
            ...
            order_percent(sid, targets['nomalized_gap'].loc[sid] * -0.5)
            logging("{0}\t{1: .5f}".format(sid.symbol, targets['nomalized_gap'].loc[sid] * - 0.5))
