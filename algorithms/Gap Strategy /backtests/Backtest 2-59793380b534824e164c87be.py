...
def initialize(context):
    context.special_sids = [sid(24), sid(42950), sid(46631)]
...
def find_gapups(context, data):
    ...
    df_gapups = df_eligibles[
        (df_eligibles.gap > 0.05)
        & (df_eligibles.gap < 1.0)
        & (df_eligibles.turnover_ratio > 0.0)
        & (df_eligibles.turnover_ratio < 1.0)
...
def get_prices(context, data, sid):
    ...
    data = data.history(sid, fields="price", bar_count=40+1, frequency="1m")
    yesterday_close = 0
    today_open = 1
    entry = today_open+1 #+2
    exit = -1
    close_price = data[yesterday_close]
    open_price = data[today_open]
...
def close_orders(context, data):
    ...
    for sid in context.portfolio.positions:
        ...
        gap, pl, exit_price = return_at_short_position_closed(context, data, sid)
        logging("{0}\t{1}\t{2: .5f}\t{3: .5f}\t{4}\t{5: .5f}\t{6: .5f}".format(side, sid.symbol, gap, pl, sector, exit_price, day_before_yesterday_close))
